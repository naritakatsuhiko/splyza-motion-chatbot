...
- **参照データ**: `src/data/knowledge/` 内の Markdown ファイル群
  - **安全性の確保**: `EXCLUDED_CONTENTS.md`（例外定義ファイル）については、チャットボットが読み込まないようシステム（`route.ts`）側で自動的に除外（スキップ）処理されます。
...
### 4.2 非公開・保留ナレッジの分離管理（バックログ）
- **保管場所**: `src/data/pending_knowledge/` フォルダ
- **目的**: 現時点では絶対にチャットボットに含めたくないが、いずれ含める可能性のある SPLYZA Motion 関連の知識（未公開仕様、社内メモ、限定マニュアルなど）を安全に隔離して保管します。
- **挙動**: システムは `src/data/knowledge/` 直下のみを参照するため、このフォルダのファイルは自動的に完全に無視され、回答に混入することはありません。
