# SPLYZA Motion Chatbot

SPLYZA Motion ヘルプセンターのナレッジをベースにした、ユーザー向けの自動応答AIチャットボットシステムです。
...
| :--- | :--- | :---: | :--- |
| **`src/data/knowledge`** | **本番公開ナレッジ**<br>（ボットが学習・回答するデータ） | **あり**<br>(自動読み込み) | ヘルプセンターから同期された操作手順や仕様などの公開データ (`01_`〜`10_`) |
...
│       │   ├── 01_Help_Center_...   # 各種公開ナレッジ (01〜10)
...
1. **[開発・運用仕様書](guidelines_and_specs.md) (`guidelines_and_specs.md`)**
   - チャットボットの基盤モデル（Gemini 3.0 Flash）、回答の生成パラメータ、回答ロジック（記載がある場合・ない場合の挙動）、有人サポートへの問い合わせ誘導の判断基準などを網羅しています。
